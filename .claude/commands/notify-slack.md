...
요약된 영상 정보를 Slack DM으로 전송합니다. (채널이 아닌 사용자 DM으로 드래프트 생성)
...
- `slack_channel_id`에는 사용자 ID(U로 시작)를 설정하여 DM으로 전송합니다.
- 채널 ID가 설정되지 않은 경우 사용자에게 먼저 설정을 요청합니다.
- 채널당 드래프트는 1개만 가능합니다. 기존 드래프트가 있으면 먼저 전송/삭제 후 재시도합니다.
